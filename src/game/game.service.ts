...
import { winningCombos } from './game.constants';
import {
  type PlayerId,
  type Game,
  type PlayerSymbol,
...
export class GameService {
  ...
  startGame(client1Id: PlayerId, client2Id: PlayerId) {
    const game: Game = {
      players: [
        { clientId: client1Id, symbol: 'X' },
        { clientId: client2Id, symbol: 'O' },
      ],
  ...
  private checkWin(board: (PlayerSymbol | null)[]): boolean {
    return winningCombos.some((combo) => {
      return (
        combo.every((index) => board[index] === 'X') ||
        combo.every((index) => board[index] === 'O')
      );
    });
  }
}
